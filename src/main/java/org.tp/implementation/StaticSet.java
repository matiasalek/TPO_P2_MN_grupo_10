package main.java.org.tp.implementation;

import main.java.org.tp.definition.Set;

import java.util.Random;

public class StaticSet implements Set {

    private static final int MAX = 10000;

    final int[] array;
    int count;

    private StaticSet(Builder builder) {
        this.array = new int[MAX];
        this.count = 0;
        // Add all elements from the builder to the set
        for (int element : builder.elements) {
            if (element != Integer.MIN_VALUE) {
                this.add(element);
            }
        }
    }

    public StaticSet() {
        this.array = new int[MAX];
        this.count = 0;
    }

    @Override
    public void add(int a) {
        if (count == MAX) {
            throw new RuntimeException("Limite de elementos alcanzado");
        }
        for (int i = 0; i < count; i++) {
            if (this.array[i] == a) {
                return;
            }
        }
        this.array[count] = a;
        this.count++;
    }

    @Override
    public void remove(int a) {
        for (int i = 0; i < count; i++) {
            if (this.array[i] == a) {
                this.array[i] = this.array[count - 1];
                count--;
                return;
            }
        }
    }

    @Override
    public boolean isEmpty() {
        return this.count == 0;
    }

    @Override
    public int choose() {
        if (this.count == 0) {
            throw new RuntimeException("No se puede elegir un valor de un conjunto vacio");
        }
        return this.array[new Random().nextInt(this.count)];
    }

    public static class Builder {
        private final int[] elements;
        private int size;

        public Builder() {
            this.elements = new int[MAX];
            for (int i = 0; i < MAX; i++) {
                this.elements[i] = Integer.MIN_VALUE; // Placeholder for empty spots
            }
            this.size = 0;
        }

        public Builder add(int a) {
            if (size >= MAX) {
                throw new RuntimeException("Limite de elementos alcanzado");
            }
            for (int i = 0; i < size; i++) {
                if (this.elements[i] == a) {
                    return this;
                }
            }
            this.elements[size++] = a;
            return this;
        }

        public Builder addAll(Set otherSet) {
            StaticSet otherStaticSet = (StaticSet) otherSet;
            for (int i = 0; i < otherStaticSet.count; i++) {
                this.add(otherStaticSet.array[i]);
            }
            return this;
        }

        public StaticSet build() {
            return new StaticSet(this);
        }
    }
}
